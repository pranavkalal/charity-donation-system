...
const MockPaymentForm = () => {
  const { slug } = useParams(); // campaign identifier from URL
  ...
  const handleSubmit = (e) => {
    ...
    alert(`✅ You donated $${finalAmount} to "${slug}" via ${paymentMethod}`);

    // Optionally redirect back to campaign details
    navigate(`/campaigns/${slug}`);
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-gray-50 py-8 px-4">
      <h1 className="text-2xl font-bold text-blue-900 mb-6">Donate to {slug.replace("-", " ")}</h1>

      <form onSubmit={handleSubmit} className="bg-white w-full max-w-xl rounded-xl shadow p-6 space-y-6">
        {/* Amount Section */}
        <div>
